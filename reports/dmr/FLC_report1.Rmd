...
```{R, BKCcalculations}
	car250_psc_crab2015 <- read.csv("https://alaskafisheries.noaa.gov/sites/default/files/reports/car250_psc_crab2015.csv",skip=6)
	...
	bkc2 <- bkc %>% filter(WeekNumber %in% .WEEK-1, Year %in% .YEAR) %>% group_by(Year,AREA) %>% summarise(NumberOfCrab=sum(NUMBER.OF.CRAB))  %>% spread(Year,NumberOfCrab)

	colnames(bkc2) <- c("Area",paste0("Week ",.WEEK-1," (",.YEAR,")"))
...
Total number of blue king crab caught to date in `r year(.TODAY)` is `r prettyNum(crab2016,big.mark=",")  `. In `r year(.TODAY)-1` the total number of blue king crab was `r prettyNum(bkc1[1,2],big.mark=",")`.
